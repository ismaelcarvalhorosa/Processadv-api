...
public class ExploradorService {
	...
	private void validar(Explorador explorador) {
		Set<ConstraintViolation<Object>> validate = validator.validate(explorador);
		if (!validate.isEmpty()) {
			throw new ConstraintViolationException(validate);
		}
	}
	
	public List<Explorador> tudo() {
	...
	public Explorador busca(Long id) {
		return em.find(Explorador.class, id);
	}
	...
	public Mensagem validarExclusao(String codigo) {
		//explorador em outros cadastros
        ...
        "WHERE constraint_type = 'FOREIGN KEY' AND ccu.table_name='explorador'";
	...
	public Mensagem validarExplorador(Explorador explorador) {
		...
		if (explorador.getExpcodigo().intValue()>0)
			sql += " and expcodigo<>"+String.valueOf(explorador.getExpcodigo().intValue());
