...
public class DistritoService {
	...
	private void validar(Distrito distrito) {
		Set<ConstraintViolation<Object>> validate = validator.validate(distrito);
		if (!validate.isEmpty()) {
			throw new ConstraintViolationException(validate);
		}
	}
	
	public List<Distrito> tudo() {
	...
	public Mensagem validarExclusao(String codigo) {
        ...
        "WHERE constraint_type = 'FOREIGN KEY' AND ccu.table_name='distrito'";
	...
	public Mensagem validarDistrito(Distrito distrito) {
		Mensagem msg = new Mensagem();
		Session session = this.em.unwrap(Session.class);
		String sql = "select * from distrito where upper(disnome) = '"+distrito.getDisnome().toUpperCase()+"'";
		if (distrito.getDiscodigo().intValue()>0)
